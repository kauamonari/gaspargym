...
import { STORAGE_KEYS, storage, type WeightEntry } from "@/storage/storage";
import {
  LineChart,
  Line,
  XAxis,
      ...
      <SurfaceCard className="space-y-2">
        ...
        <div className="flex items-end justify-between">
          ...
          >
            ...
            <span className="tabular-nums">{diff > 0 ? "+" : ""}{diff} kg</span>
          </div>
        </div>
      </SurfaceCard>
          ...
          <div className="h-52 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
                ...
                <YAxis stroke="var(--color-muted-foreground)" fontSize={11} tickLine={false} axisLine={false} domain={["dataMin - 1", "dataMax + 1"]} />
                <Tooltip
                  contentStyle={{
                ...
                />
                <Line type="monotone" dataKey="peso" stroke="url(#lineG)" strokeWidth={3} dot={{ r: 4, fill: "oklch(0.82 0.19 142)" }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
            ...
            {[...sorted].reverse().map((e) => (
              <li key={e.id}>
                <SurfaceCard className="flex items-center justify-between p-4">
                  <div>
                    <p className="font-display text-lg font-bold tabular-nums">{e.weight.toFixed(1)} kg</p>
