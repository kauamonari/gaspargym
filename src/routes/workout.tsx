...
export const Route = createFileRoute("/workout")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Organize seus treinos em blocos e acompanhe a evolução de carga." },
    ],
  }),
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <Tabs defaultValue="registrar" className="w-full">
        ...
        <TabsContent value="registrar" className="mt-4 space-y-6">
          <RegisterTab sets={sets} blocks={blocks} onPersistSets={persistSets} onPersistBlocks={persistBlocks} />
        </TabsContent>
...
function RegisterTab({
  ...
  const todayForBlock = activeBlock
    ? todaySets.filter((s) => s.blockId === activeBlock.id)
    : [];
  ...
  const pickerResults = useMemo(() => {
    ...
    const pool = EXERCISES.filter((e) => e.grupo === group);
    if (!n) return pool;
  ...
  function toggleExerciseInBlock(exerciseId: number) {
    ...
    onPersistBlocks(blocks.map((b) => (b.id === activeBlock.id ? { ...b, exerciseIds: nextIds } : b)));
  }
          ...
          {managing ? (
            <div className="space-y-4">
              ...
              <ul className="space-y-2">
                {EXERCISES.filter((e) => e.grupo === group).map((e) => {
                  const included = activeBlock.exerciseIds.includes(e.id);
                  return (
                    <li key={e.id}>
                      <button onClick={() => toggleExerciseInBlock(e.id)} className="w-full text-left">
                        <SurfaceCard
            ...
            <>
                ...
                <SurfaceCard className="space-y-4 border-workout/40">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="block">
                      <span className="text-xs uppercase tracking-wider text-muted-foreground">Carga (kg)</span>
                      <input
                        type="number"
                        min={0}
                        step="0.5"
                    ...
                    <label className="block">
                      <span className="text-xs uppercase tracking-wider text-muted-foreground">Repetições</span>
                      <input
                        type="number"
                        min={1}
                        value={reps}
                              ...
                              <SurfaceCard className="flex items-center justify-between p-4 transition-colors hover:border-workout/40">
                                ...
                                {doneToday ? (
                                  <span className="text-xs font-medium text-workout">
                                    {doneToday.length} {doneToday.length === 1 ? "série" : "séries"} · {maxCarga(doneToday)}kg
                                  </span>
                                ) : (
                                  <Plus className="h-4 w-4 text-muted-foreground" />
                                )}
              ...
              {grouped.size > 0 && (
                <section className="space-y-3">
                  <h2 className="font-display text-lg font-semibold">Séries de hoje — {activeBlock.label}</h2>
                      ...
                      <SurfaceCard key={name} className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <p className="text-[11px] uppercase tracking-wider text-muted-foreground">
                            {items.length} {items.length === 1 ? "série" : "séries"} · máx {maxCarga(items)}kg
                          </p>
                        </div>
...
function EvolutionTab({ sets }: { sets: WorkoutSet[] }) {
  const names = distinctExerciseNames(sets);
  const [exerciseName, setExerciseName] = useState<string | null>(names[0] ?? null);

  useEffect(() => {
    if (!exerciseName && names.length > 0) setExerciseName(names[0]);
  }, [names, exerciseName]);
  ...
  return (
    <>
      ...
      <SurfaceCard className="space-y-2">
        <div className="flex items-baseline justify-between">
          <span className="text-xs uppercase tracking-wider text-muted-foreground">Carga máxima atual</span>
        </div>
              ...
              >
                <defs>
                  <linearGradient id="loadG" x1="0" y1="0" x2="1" y2="0">
                    ...
                    <stop offset="100%" stopColor="oklch(0.55 0.19 150)" />
                  </linearGradient>
                </defs>
                ...
                />
                <Line type="monotone" dataKey="carga" stroke="url(#loadG)" strokeWidth={3} dot={{ r: 4, fill: "oklch(0.6 0.19 150)" }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
